...
export const useRoomStore = defineStore('rooms', () => {
  ...
  const roomsWithCategory = computed<RoomWithCategory[]>(() => rooms.value.map((room) => ({
    id: room.id,
    name: room.name,
    status: room.status,
  ...
  const getById = (id: number): Room | undefined => rooms.value.find((room) => room.id === id);

  const getCategoryById = (id: number): Category | undefined => categories.value.find((category) => category.id === id);
